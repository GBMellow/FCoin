...
class Validar:
    def validar_transacao(self, validador, remetente_id, valor):
        ...
        validador["ultima_transacao"] = self.converter_data(
            validador["ultima_transacao"]
        )
        if (
            validador["ultima_transacao"] is not None
            and horario_atual <= validador["ultima_transacao"]
        ):
            return False
    ...
    def concluir_transacao(self, transacao, validador):
        ...
        if self.validar_transacao(
            validador, transacao["remetente"], transacao["valor"]
        ):
